...
if (isset($_POST['validar'])) {
    ...
    $stmt = mysqli_prepare($enlace, "UPDATE reservas SET idEstado = 2 WHERE idReserva = ?");
    if ($stmt === false) {
    ...
    if ($execute) {
        echo "<script>alert('Reserva validada correctamente');</script>";
    } else {
        echo "<script>alert('Error al validar la reserva');</script>";
    }
